...
/*   Updated: 2017/10/17 23:55:44 by oukrifa          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	size_t	put_wchar(t_flag *env, wchar_t c)
{
	size_t		u_fmt;
	size_t		tmp_mask;
	int			nboctet;
	...
	{
		if (env->precision-- > 0)
			add_to_buff(env, (char)(u_fmt >> (8 * (nboctet - 1))));
		nboctet--;
	}
	return (nboctet);
}
...
static	void	ft_put_space(int call, int s_len, int len, t_flag *env)
{
	if (call == 1)
	{
		while (!env->flag['-'] && !env->flag['0'] && WIDTH >
				(env->precision > len ? env->precision : len) + s_len)
			add_to_buff(env, ' ');
	}
	else if (call == 2)
	{
...
static	size_t	ft_put_value(t_flag *env, wchar_t *p)
{
	size_t	ret;
...
void			put_ls(wchar_t *p, int len, t_flag *env)
{
	WIDTH ? ft_put_space(1, 0, len, env) : 0;
	if (ft_put_value(env, p) == 0x100)
	{
		env->n_printed = -1;
		env->i--;
		return ;
	}
	WIDTH > 0 ? ft_put_space(2, 0, len, env) : 0;
}
